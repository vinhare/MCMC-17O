function [Deltap, deltap, rh_frac] = Body_Water_Model_rh(DeltaOatm, deltaOmw, rls, WEIndex)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
WEI=WEIndex;                   % Water economy index (ml/KJ) for ostrich (Williams et al., 1993)
...
rh=i*0.001;                    % Relative humidity
T=273.15+15;                % Enviroment temperature (K)
d18O_mw=deltaOmw;                 % Meteoric water d18O composition (per mil) Namib
D17O_mw=0.09;               % Meteoric water D17O composition (per mil)
d18O_atmO2=23.881;          % Atmospheric O2 d18O composition (23.881, Barkan and Luz, 2011)
D17O_atmO2=DeltaOatm;          % Atmospheric O2 D17O composition (-0.506, Barkan and Luz, 2011)
...
Leaf_ratio=rls;            % Ingested leaf ratio relative to total ingested vegetation
...
a_O2carb_H2Ocarb_18 = 1.038;                      % Passey et al 2014, Table 3
l_O2carb_H2Ocarb = 0.5245;                        % Passey et al 2014, Table 3 footnotes
...
if H2O_in_nodrink > WEI_turnover
    ...
    r(i)=1;                 % r=1, WEI not determine water turnover
else
    H2O_turnover=WEI_turnover;
    r(i)=0;                 % r=0, WEI determine water turnover
end

if WEI_turnover-Air_H2O-Food_H2O_assoc-Food_H2_content>0
    Drinking=WEI_turnover-Air_H2O-Food_H2O_assoc-Food_H2_content;
    t(i)=1;                 % t=1, Drink water  **** note here change Food_H2_content into Food_O2_content ****
else
    Drinking=0;
    t(i)=0;                 % t=0, Do not drink water
end
...
d17O_bw=(R_17_bw/R_17_SMOW-1)*1000;
                            % d17O of body water
deltap(i,1)=1000*log(d18O_bw/1000+1);     % Prime 
Deltap(i,1)=1000*log(d17O_bw/1000+1)-lamda*1000*log(d18O_bw/1000+1);
                            % D17O of body water
...
%%% Drake has edited this a bit %%% ...he says to himself...
% Equivalent carbonates information
%a_O2carb_H2Ocarb_17 = exp(l_O2carb_H2Ocarb*reallog(a_O2carb_H2Ocarb_18));
 
%d18O_carb_SMOW(i,1)=a_O2carb_H2Ocarb_18*(d18O_bw+1000)-1000;       % Carbonate d18O in SMOW... was 1.027
%d18O_carb_PDB(i)=(d18O_carb_SMOW(i)-30.86)/1.03086;   % carbonate d18O in PDB
%  
%d17O_carb_SMOW(i,1)=a_O2carb_H2Ocarb_17*(d17O_bw+1000)-1000;   % Calculate d17O from d18O & alpha
%dp17O_carb_SMOW=1000*(log(d17O_carb_SMOW/1000+1));    % Prime it
%deltap=1000*(log(d18O_carb_SMOW/1000+1));     % Prime 
%  
%Deltap=(dp17O_carb_SMOW-lamda*deltap);   % Ta-fucking-da 
rh_frac(i,1)=rh;

%Rel_Humidity(i,1)=0.1*i;
end
end 
%%%%%%%%%%%%%%%%
%     Plots    %
%%%%%%%%%%%%%%%%%
%scatter(d18O_carb_SMOW,D17O_carb_SMOW,20,Rel_Humidity,'filled');
%colorbar;
%box on;
