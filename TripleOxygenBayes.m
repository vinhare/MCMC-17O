...
% 24 March 2025 - Patched the bootstrap to allow for variable NAME.xdata
% length
%% 
...
load TripleO_Southern_MCO.mat % a .mat file with data.ydata, data.xdata
data=Southern_MCO.data;
...
% The following lines define the PRIORS for theta, as well as whether they
% are targets for the MCMC (target? = 1 means that the posterior 
% distribution will be estimated from the prior. target? = 0 indicates 
% marginalised prior (i.e. the posterior is not estimated).    
...
width = 3 %3.4252;     % Width in inches. 3 is good for multifigures. 
%3.4252 is golden ratio
...
for i = 1:numBoot;
    ...
    data_boot(:,1) = (1:size(data_boot,1))'; 
